...
### The banks section (`/banks`) — data in `lib/banks.ts` + `lib/banks/<slug>.ts`

One page per bank on its AI strategy, from public sources only. `lib/banks.ts` holds the
types and assembles `BANKS` from one module per bank in `lib/banks/`. Each bank carries a
`sources[]` list (id, title, publisher, url, date) and every dated entry — timeline,
use cases, numbers, quotes, leadership, platform — cites source ids. `npm run tracker:check`
(`checkBanks`) fails on: an uncited entry, an unknown source id, a source on a host outside
the allowed set (regulator, bank/vendor primary, trade press, and the tier-1 business press
listed in `BANK_PRESS_HOSTS`), an unknown regulator/document slug, or a page thinner than the
minimums (answer ≥ 400 chars, ≥ 4 key points, ≥ 6 timeline entries, ≥ 3 suggestions,
≥ 2 FAQ). `--bank-links` adds the bank sources to the live link check. Bump the bank's
